...
module.exports = function httpAdapter(resolve, reject, config) {
  ...
  //Through Proxy
  var proxy =  process.env && process.env.http_proxy;
  var parsedProxy, proxyPort, auth, proxyHost, proxyHeaders;

  if(!!proxy) {
    parsedProxy = url.parse(proxy);
    proxyPort = parsedProxy.port || 80;
    auth = parsedProxy.auth ? 'Basic ' + new Buffer(parsedProxy.auth).toString('base64') : undefined;
    ...
    options.headers =  utils.merge(options.headers,proxyHeaders);
